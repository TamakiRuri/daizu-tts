...
```
    ...
    /auto //自動接続の有効/無効の切り替え（サーバー全体の読み上げチャンネルもこのチャンネルに設定されます）
    /setnotifychannel //ボイスチャット通知を有効にし、通知チャンネルを現在のチャンネルに設定する（自動入室が必須です）
    /resetnotifychannel //通知を無効にする
    /connect  //ボイスチャットに接続する
    /disconnect  //ボイスチャットから切断する
